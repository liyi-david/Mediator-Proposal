...
The algorithm has the following steps. Firstly, \liyi{each process sends a voting message including its own \emph{id} to its successor. Then when a process receives a voting message, it will
	\begin{itemize}
		\item forward the message to its successor if it contains a larger \emph{id} than itself, or
		\item ignore the message if it contains a smaller \emph{id} than itself, or
...
The following code fragment encodes a parallel program containing 3 \emph{worker}s and 3 \emph{election\_module}s to organize the \emph{worker}s.  In this example, we do not focus on the implementation details on \emph{worker}s, \liyi{but hope that any component with a proper interface could be embedded into this system instead}. 
